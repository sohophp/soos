...
export const pageSpeedText = {
  en: {
    ...
    onDemand: "on demand", apiKey: "Google API key", apiKeyPlaceholder: "Stored for this browser tab only",
    apiKeyHelp: "Enable PageSpeed Insights API. Enable Chrome UX Report API to load the optional CrUX data below.",
    ...
    privacy: "The selected URL and API key are sent through the soos server to Google PageSpeed Insights and, when selected, Chrome UX Report. The key is kept only in session storage and is not saved in reports, history, logs, or Neon.",
  ...
  "zh-CN": {
    ...
    onDemand: "按需运行", apiKey: "Google API 密钥", apiKeyPlaceholder: "仅在当前浏览器标签页保存",
    apiKeyHelp: "需启用 PageSpeed Insights API；如需加载下方可选的 CrUX 数据，还要启用 Chrome UX Report API。",
    ...
    privacy: "所选网址和 API 密钥会经 soos 服务端发送给 Google PageSpeed Insights；勾选后也会发送给 Chrome UX Report。密钥仅保存在会话存储，不写入报告、历史、日志或 Neon。",
  ...
  "zh-TW": {
    ...
    onDemand: "按需執行", apiKey: "Google API 金鑰", apiKeyPlaceholder: "僅在目前瀏覽器分頁儲存",
    apiKeyHelp: "需啟用 PageSpeed Insights API；如需載入下方可選的 CrUX 資料，還要啟用 Chrome UX Report API。",
    ...
    privacy: "所選網址與 API 金鑰會經 soos 伺服器傳送給 Google PageSpeed Insights；勾選後也會傳送給 Chrome UX Report。金鑰僅存於工作階段儲存空間，不寫入報告、歷史、日誌或 Neon。",
