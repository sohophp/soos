...
- 增加日、周和每 30 天重复扫描计划，计划按浏览器会话隔离并保存在 Neon。
- 增加由 `CRON_SECRET` 保护的 `/api/cron/audits` 入口，每次最多推进 3 个计划和每个任务 20 个 URL 批次。
- 定时任务沿用 checkpoint、worker lease 和中断恢复，错误任务不会丢弃已完成批次。
- Vercel 默认每天 UTC 03:00 触发 Cron，Serverless 函数最大时限配置为 300 秒。
- 定时扫描强制使用标准 250 URL 模式并关闭性能检查，避免无人值守任务产生过高负载。
- Hobby 每日 Cron 适合轻量日/周/月监控；大型或高频监控可由外部 Cron 使用同一受保护端点。
- 完成 Milestone F：运营能力。
