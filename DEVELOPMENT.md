...
- URL 标准化、查询参数、尾斜杠、大小写和重定向链需要统一策略。
...
- 增加可选的站内递归发现队列，并设置域名、深度、URL 数量、并发和时间预算。
- 建立 URL 节点与链接边，计算入链数、抓取深度、孤立页和仅站内发现页。
...
- 接入 GSC Sitemaps API，展示 Google 看到的 sitemap 状态、提交时间和错误。
