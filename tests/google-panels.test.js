...
assert.match(inspectionSource, /requestIdRef/);
assert.match(inspectionSource, /siteUrl\.trim\(\) !== requestedSiteUrl/);
assert.match(inspectionSource, /if \(requestId === requestIdRef\.current\) setLoading\(false\)/);
...
assert.doesNotMatch(sitemapSource, /key=\{item\.path\}/);
assert.match(sitemapSource, /key=\{`\$\{item\.path\}-\$\{item\.type \|\| ""\}-\$\{index\}`\}/);
assert.doesNotMatch(configSource, />\{copy\.refresh\}</);
assert.doesNotMatch(configSource, />\{testing \? copy\.testing : copy\.test\}</);
assert.doesNotMatch(configSource, /key=\{site\.siteUrl\}/);
assert.match(configSource, /const propertyOptions = useMemo/);
assert.match(configSource, /bySiteUrl\.has\(optionSiteUrl\)/);
assert.match(configSource, /propertyOptions\.map/);
assert.match(configSource, /key=\{`gsc-property-\$\{index\}-\$\{site\.siteUrl\}`\}/);
assert.match(configSource, /developers\.google\.com\/webmaster-tools\/v1\/how-tos\/authorizing/);
assert.match(configSource, /console\.cloud\.google\.com\/apis\/library\/searchconsole/);
assert.match(configSource, /developers\.google\.com\/search\/docs\/monitor-debug\/search-console-start/);
assert.match(analyticsSource, /if \(loadedDimension === "page"\) onRows\(loadedRows\)/);
